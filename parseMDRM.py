# -*- coding: utf-8 -*-
"""
Created on Thu Mar 27 21:33:50 2025
...
def init_argparse() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        usage="%(prog)s",
        description="Downloads the latest data dictionary ZIP file from the Federal Reserve, and converts it to a JSON records representing the data dictionary.",
    )
    parser.add_argument(
...
def main():
    ...
    print("Starting data dictionary download...")
    # collect the latest data dictionary zip file, and return a byte array representing the CSV file
    csv_bytes = collect_latest_data_dictionary_zip()
    print("Data dictionary download complete.")

    print("Starting data dictionary processing...")

    # process the CSV file and return a Pandas DataFrame
    df = process_csv(csv_bytes)

    print("Data dictionary processing complete.")


    # get the current date and time
    current_datetime = datetime.now()

    # write the current datetime to a string
    current_datetime_str = current_datetime.strftime('%Y-%m-%d')

    # create the filename
    output_file_name = f"mdrm_data_dictionary_{current_datetime_str}.json"
    ...
    # print when the script finished
